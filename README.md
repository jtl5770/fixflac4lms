...
**fixflac4lms** is a specialized tool designed to correct specific FLAC metadata issues that can cause problems with [Lyrion Music Server (LMS)](https://lyrion.org/).
...
1.  **Merge MusicBrainz IDs:** It detects multiple instances of specific ID tags (`MUSICBRAINZ_ARTISTID`, `MUSICBRAINZ_ALBUMARTISTID`, `MUSICBRAINZ_RELEASE_ARTISTID`) and merges them into a single tag with values separated by `+`. This prevents the LMS bug while preserving the data.
2.  **Embed Cover Art:** It checks for embedded cover art. If missing, it looks for a `cover.jpg` file in the same directory and embeds it.
...
By default, the tool runs in **dry-run** mode. It will tell you what needs to be changed but will not modify any files.
...
To actually modify files, you must use the `-w` (write) flag along with the specific feature flags you want to enable.
